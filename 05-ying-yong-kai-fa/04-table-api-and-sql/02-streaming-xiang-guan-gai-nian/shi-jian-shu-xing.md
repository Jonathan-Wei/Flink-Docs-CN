...
有关Flink中时间处理的更多信息，请参阅有关[事件时间和水位线的简介](https://ci.apache.org/projects/flink/flink-docs-release-1.7/dev/event_time.html)。
...
只要时间属性没有被修改，并且只是从查询的一个部分转发到另一个部分，它就仍然是一个有效的时间属性。时间属性的行为类似于常规的时间戳，可用于计算。如果在计算中使用时间属性，它将被具体化并成为一个常规的时间戳。常规的时间戳与Flink的时间和水位线系统不兼容，因此不能再用于基于时间的操作。
...
处理时间允许表程序根据本地机器的时间生成结果。它是时间最简单的概念，但不提供决定论。它既不需要提取时间戳，也不需要生成水位线。
...
为了处理无序事件和区分流中的准时事件和延迟事件，Flink需要从事件中提取时间戳并在时间上取得某种进展\(即所谓的水位线\)。

可以在创建表DDL中或在DataStream到Table的转换过程中或使用TableSource定义事件时间属性。

### 在创建表DDL中定义

 事件时间属性是使用CREATE TABLE DDL中的WATERMARK语句定义的。水位线语句在现有事件时间字段上定义水位线生成表达式，该表达式将事件时间字段标记为事件时间属性。有关水位线声明和水位线策略的更多信息，请参见[CREATE TABLE DDL](https://ci.apache.org/projects/flink/flink-docs-release-1.10/dev/table/sql/create.html#create-table)。
...
事件时间属性是在模式定义期间使用`.rowtime`属性定义的。[时间戳和](https://ci.apache.org/projects/flink/flink-docs-release-1.7/dev/event_time.html)水位线必须在转换的数据流中分配。
...
事件时间属性由实现`DefinedRowtimeAttributes`接口的TableSource定义。`getRowtimeAttributeDescriptors()`方法返回一个`RowtimeAttributeDescriptor`列表，用于描述时间属性的最终名称、用于派生属性值的时间戳提取器和与属性关联的水位线策略。

请确保`getDataStream()`方法返回的`DataStream`与定义的`time`属性对齐。`DataStream`的时间戳\(由`TimestampAssigner`分配的时间戳\)只在定义了`StreamRecordTimestamp`时间戳提取器时才会考虑。只有定义了保留水位线策略，数据流的水位线才会被保留。否则，只有`TableSource`的`rowtime`属性的值是相关的。
